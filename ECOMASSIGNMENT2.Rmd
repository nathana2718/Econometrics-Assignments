...
For unitary $\sigma$, the interval length is far smaller than their $\sigma=2$ counterparts. 
...
And so depends on $\sigma$. In this case as $X_3$ is unobservable, it simply adds to the noise, thereby increasing variance. 
...
Interval lengths are also enlarged as a result of increased standard deviations.
...
Ths reason why the old $Y_{hat}$ is roughly the same value as the new one is because no new dimensions have really been added to the column space - X_2 is simply added again and as  a projection is a linear combination, this can be compensated for very precisely by simply changing $\beta_2$.

Therefore $\beta_1$ remains largely unchanged throughout changes in $\alpha_2$.
